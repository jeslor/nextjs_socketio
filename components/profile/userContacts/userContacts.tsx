      ...
      {myContacts.length ? (
        <div className="flex items-center justify-start gap-5 flex-wrap">
          {myContacts.map((contact: any) => (
            <div
              key={contact._id}
              className="flex flex-col items-center justify-center relative group border-[1px] border-black/5  w-[200px] h-[200px] bg-white/10 rounded-[10px] p-2"
            >
              {contact.profileImage &&
              !contact.privacySettings.hideProfilePhoto ? (
                <div className="size-18 bg-base-100 p-2 rounded-full flex items-center justify-center relative">
                  ...
                  {onlineContacts.includes(contact._id) &&
                    !contact.privacySettings.hideOnlineStatus && (
                      <span className="absolute h-3 w-3 rounded-full bg-green-700 top-3 -right-[1px] border-[1px] border-green-300"></span>
                    )}
                  <img
                    src={contact.profileImage}
                    alt=""
                    className="size-[90%] rounded-full object-cover object-top"
                  />
                </div>
              ) : (
              ...
              <button
                ...
                className="py-1 px-2 bg-primary/80 hover:bg-primary mt-2 rounded-[5px] text-[11px] text-base-100 font-semibold cursor-pointer"
              >
                remove
              </button>
            </div>
