...
const ChatContacts = memo(
  ({ expand, searchQuery }: { expand: boolean; searchQuery: string }) => {
    ...
    useEffect(() => {
      let currentUserContactIds = currentUser?.contacts.map(
        (contactId: any) => contactId._id
      );
      if (contacts.length > 0) {
        if (searchQuery.length > 0) {
            ...
            contacts.filter((contact: any) => {
              return (
                (contact.username
                  ...
                  contact.email.toLowerCase().includes(LowerCaseSearchQuery)) &&
                currentUserContactIds.includes(contact._id)
              );
            })
          );
          setOtherContacts(
          ...
          setOtherContacts(
            contacts.filter(
              (contact: any) => !currentUserContactIds.includes(contact._id)
            )
          );
        }
      }
