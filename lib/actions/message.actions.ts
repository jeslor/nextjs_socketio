...
export const newMessage = async ({
  ...
  try {
    ...
    if (file) {
      await uploadCloudinary(file, "message")
        .then((result) => {
    ...
    await message.save();
    const updatedMessage = await Message.findOne({ _id: message._id }).populate(
      "sender receiver"
    );
    ...
    return JSON.parse(
      JSON.stringify({
        status: 500,
        message: "Internal Server Error new message",
        data: error,
      })
    );
  }
  ...
  try {
    ...
    const messages = await Message.find({
      $and: [
        ...
        { deleted: false },
        { $or: [{ sender: loggedInUserId }, { seen: true }] },
      ],
    }).populate("sender receiver");
    return JSON.parse(
      JSON.stringify({ status: 200, message: "Messages Found", data: messages })
    );
  } catch (error) {
    console.log(error);
    return JSON.parse(
...
export const unReadMessages = async (loggedInUserId: string, message: any) => {
  try {
    ...
    return JSON.parse(
      JSON.stringify({
        status: 200,
        message: "Message added to unread",
    ...
    return JSON.parse(
      JSON.stringify({
        status: 500,
        message: "error in adding message to unread",
        data: error,
      })
    );
  }
...
export const updateReadMessages = async (
  ...
  try {
    ...
    if (!updatedLoggedInUser) {
      return JSON.parse(
        JSON.stringify({ status: 404, message: "User not found", data: null })
      );
    }
    ...
    if (updateReadMessages.modifiedCount === messageIds.length) {
      return JSON.parse(
        JSON.stringify({
          status: 200,
          message: "All messages updated to seen",
          data: updatedLoggedInUser,
        })
      );
    }
    if (updateReadMessages.modifiedCount > 0) {
      return JSON.parse(
        JSON.stringify({
          status: 200,
          message: "Some messages updated to seen",
          data: updatedLoggedInUser,
        })
      );
    }
    ...
    return JSON.parse(
      JSON.stringify({
        status: 500,
        message: "Internal Server Error update read messages",
        data: error,
      })
    );
  }
